 ...
 */
export const isSelectToolActive = derived(
    activeTool,
    $tool => $tool === 'select'
);
 ...
 */
export const isCreateToolActive = derived(
    activeTool,
    $tool => ['rectangle', 'circle', 'ellipse', 'line', 'text', 'polygon', 'star', 'image'].includes($tool)
);

/**
 * Is pan tool active?
 */
export const isPanToolActive = derived(
    activeTool,
    $tool => $tool === 'pan'
);
